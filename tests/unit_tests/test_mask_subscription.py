...
def product_datetime(product_name):
    """takes in product time; uses regex to take out the date/time of the file name
       then returns a date time object of both the start and end times.
       Returns in the (year, month, day) (hour, minute, second) format"""
...
# TODO: order by middle time instead; Maybe
def triage_products(products):
    ...
    return sorted(product_list_unsorted_1, key=lambda product: product_datetime(product['name'])[0])
...
def test_product_datetime():
    ...
    file_name = "S1A_IW_GRDH_1SDV_20190903T023801_20190903T023826_028851_0344EE_3AF9-RESORB-30m-amp-rtc-gamma.zip"
    
    start = datetime(2019, 9, 3, 2, 38, 1) 
    end = datetime(2019, 9, 3, 2, 38, 26)

    assert product_datetime(file_name) == (start, end)
...
for p in product_list_unsorted_1:
    # print(p["name"])
    print(f"""start time: {product_datetime(p['name'])[0]}""")
    # print(f"""time elapsed: {product_datetime(p['name'])[1] - product_datetime(p['name'])[0]} \n""")

    # print(f"""{product_datetime(p['name'])[0]} ==> {product_datetime(p['name'])[1]}________{product_datetime(p['name'])[1] - product_datetime(p['name'])[0]}""")



product_list_sorted_1 = sorted(product_list_unsorted_1, key=lambda product: product_datetime(product['name'])[0])
